...
if __name__ == "__main__":
    ...
    #Make lines so that they occupy half of the entire space between them and the next line
    note_groups = []
    for box in staff_boxes:
        ...
        while(i < len(staff_notes)):
            if (staff_notes[i].rec.x > staffs[j].x and j < len(staffs)):
                r = staffs[j]
                j += 1
                if len(note_group) == len(staff_notes):
                ...
                i += 1
                print("NOTE #"+str(i)+" ADDED TO GROUP")
        note_groups.append(note_group)
